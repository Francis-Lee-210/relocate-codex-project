...
Compatibility entries record both raw link text and resolved targets, so changes hidden in external aliases are compared too. Older plans with compatibility entries that lack resolved targets cannot prove this match. While the original directory is still at its original path, generate and review a fresh plan. If the move already started, retain the original plan, receipt, and paths for manual investigation using trustworthy pre-move evidence; do not fill in missing historical targets from the current filesystem or regenerate the plan at the destination. Plans with no compatibility entries remain usable when their current audit is also empty.
